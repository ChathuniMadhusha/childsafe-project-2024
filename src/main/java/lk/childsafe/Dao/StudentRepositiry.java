...
public interface StudentRepositiry extends JpaRepository <Student,Integer>{
    ...
    @Query(value = "select s from Student s where s.id in (select p.student_id.id from Parent p where p.nic=?1 and p.parent_status_id.id=1) and s.student_status_id.id=1")
