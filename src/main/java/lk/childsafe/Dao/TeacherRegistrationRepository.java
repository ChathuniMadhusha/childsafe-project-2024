...
//    TeacherRegistration findduplicateregByTID(@Param("teacherid") String teacherid, @Param("class_code") String class_code);
    ...
    @Query(value = "select new TeacherRegistration (count(tr.id)) from TeacherRegistration tr where tr.teacher_id.id=?1")
    TeacherRegistration getClassbyTeacher(Integer teache_id);

    //get registered class list according to teacher
    @Query(value = "SELECT teareg FROM TeacherRegistration teareg where teareg.teacher_id.id=?1")
    ...
    //query for get teacher clz reg list for given Class id(this is for update teacher clz reg status according to teacher status)
    @Query(value = "SELECT tr FROM TeacherRegistration tr where tr.class_implementation_id.id=?1")
    List<TeacherRegistration> getTeacherClassRegistrationsByClzId(Integer id);
